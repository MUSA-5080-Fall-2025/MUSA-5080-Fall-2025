...
crimes.sf <- bostonCrimes %>%
  ...
  st_transform('ESRI:102286') %>%  #unit is in meter
  distinct()
...
boston.sf <- boston.sf %>%
  mutate(
    crimes.Buffer = lengths(st_intersects(
      st_buffer(geometry, 660),  # unit is in meter
      crimes.sf
    )),
    crimes_500m = lengths(st_intersects(
      st_buffer(geometry, 500),   # unit is in meter in ESRI:102286 projected coordinate system
      crimes.sf
    ))
  )
...
::: {.callout-note}
Interpretation: crime_nn3 = 83.29 means the average distance to the 3 nearest crimes is 83.29 meters
:::
...
boston.sf <- boston.sf %>%
  mutate(
    dist_downtown_m = as.numeric(st_distance(geometry, downtown))
  )

# Summary
summary(boston.sf$dist_downtown_m)
```
...
spatial_summary <- boston.sf %>%
  st_drop_geometry() %>%
  select(crimes.Buffer, crimes_500m, crime_nn3, dist_downtown_m) %>%
  summary()
...
feature_summary <- tribble(
  ...
  "crimes_500m", "Buffer count", "Crimes within 500 meters",
  "crime_nn3", "kNN distance", "Avg distance to 3 nearest crimes (meters)",
  "dist_downtown_m", "Point distance", "Meters from downtown Boston"
)
...
model_spatial <- lm(SalePrice ~ LivingArea + R_BDRMS + Age +
                    crimes_500m + crime_nn3 + dist_downtown_m,
                    data = boston.sf)
...
# Model 2: + Spatial
cv_m2 <- train(
  SalePrice ~ LivingArea + Age + R_FULL_BTH + crimes_500m + crime_nn3,
  data = boston.sf, method = "lm", trControl = ctrl
)
...
cv_model_fe <- train(
  SalePrice ~ LivingArea + Age + crimes_500m + 
...
model <- train(
  SalePrice ~ LivingArea + Age + crimes_500m + as.factor(name_cv),
  data = boston.sf,
  method = "lm",
  trControl = ctrl
)
