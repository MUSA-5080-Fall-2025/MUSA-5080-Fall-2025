...
census_api_key(Sys.getenv("e17c1c6ce413dc3faca31a17a58ab3374404bb9e"))
...
household_age <- age_vars %>%
  filter(str_detect(label, "Median Age"))

print("Household age variables:")
head(household_age)

age_concept <- acs_vars_2022 %>%
  filter(str_detect(concept, "Median Household Age"))
...
ggplot(county_reliability) +
  ...
  labs(
    title = "Median Age MOE vs Population",
    x = "Total Population",
    y = "Median Age MOE (%)",
